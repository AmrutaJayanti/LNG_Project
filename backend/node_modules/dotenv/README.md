...
```
```sh
$ dotenvx run --debug -- node index.jsx
...
$ echo "console.log('Hello ' + process.env.HELLO)" > index.jsx

$ dotenvx run --env-file=.env.production -- node index.jsx
Hello production
> ^^
```
...
$ dotenvx run --env-file=.env.local --env-file=.env -- node index.jsx
Hello local
```
